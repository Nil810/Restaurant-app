...
const [totalProducts,setTotalProducts]=useState([]);
  ...
  let SelectCurrency = cur.toUpperCase()=== "INR"?"₹":"$";
    ...
    const getCatByUser =  `${baseURL}/api/categories?merchantCode=${merchantData.length ? merchantData[0].merchantCode:" "}`;
  ...
  useEffect(()=>{
    ...
    useEffect(() => {
      if (!categories.length) {
        ...
        axios.get(baseURL+`/api/categories/${categories[0]._id?categories[0]._id:categories[0].id}?merchantCode=${merchantData.length ? merchantData[0].merchantCode:" "}`)
        .then((response)=>{
    ...
    useEffect(()=>{
      ...
      axios.get(`${baseURL}/api/modifiers?merchantCode=${merchantData.length ? merchantData[0].merchantCode:" "}`)
   ...
   let productItems = isSearch?filterPro:catProducts.length?catProducts : products;
  ...
  const handleSearch =(e)=>{
      ...
      let fltData=products.filter(pro=>pro.name.toLowerCase().indexOf(val.toLowerCase()) !== -1)
      console.log(fltData);
    ...
    orde.orderItems =orde.orderItems.map(it=>{
        ...
        if(orde&&orderItem.length&&orde.orderItems.length){
          console.log("order Placed");
            axios.post(`${baseURL}/api/orders?merchantCode=${merchantData.length ? merchantData[0].merchantCode:" "}`,orde).then(res=>{
...
const handleContinue =()=>{
  ...
  const categoryClickHandler =(catName,catId,isAddOn)=>{
         ...
         if(isAddOn){
          ...
          axios.get(baseURL+`/api/categories/${catId}?merchantCode=${merchantData.length ? merchantData[0].merchantCode:" "}`)
          .then((response)=>{
            setProducts(response.data)
            console.log(response.data);
          })
...
let cat = catAddon?catAddon:categories;
    ...
    <div className="main_po">
                ...
                <div style={{padding:"0px",height:"100%"}}>
                    {
                      ...
                      <img src={selectPro.image===""?"../images/blank.jpeg":baseURL+'/'+selectPro.image}
                      ...
                      <div style={{display:"flex",justifyContent:"space-around",fontSize:"20px"}}> 
                          ...
                          <span>{SelectCurrency}{selectPro.price}</span>
                      </div>
              </div>
                      :""
                    }
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <button className="btn btn-danger btn-small m-2"
                      onClick={()=>{
                        setSelectPro();
                        setProOpen(false);}
                      }
                        >Close</button>
                      <button className="btn btn-info btn-small m-2" onClick={handleItem}>Add</button>
                    </div>
                </div>
        </Dialog>
        <Dialog
        ...
        >
          ...
          style={{width:"252px",height:"280px",display:"flex",flexDirection:"column", justifyContent:"space-between",alignItems:"center"}}
>
            <CardMedia 
                            component="img"
                            alt={product.name}
          ...
          <Box id='adionorder' style={{margin:"10px"}}>
            ...
            { addons.length>=1?addons.map(li=> (
              <div id="addons" style={{display:"flex",justifyContent:"space-between",alignContent:"center",padding:"3px 15px",fontSize:"20px",flexWrap:"wrap"}}>
                <span id='addname'> {li.name} </span><div className={styles.btn_group} id='btn_group'>
                <span id='pri'>{selectedCurrency} {li.price} </span>
                            <span className={styles.addons} id='addonsbtn'>
                            <button className={styles.minus1} onClick={()=>removeAddons(li._id?li._id:li.id)}><RemoveIcon /></button>
                            {li.quantity} 
                            <button className={styles.plus} onClick={()=>adAddons(li._id?li._id:li.id)}> <AddIcon sx={{ fontSize:"1px"}} /></button>
                            </span>
                           </div> 
              </div> 
            ))
        ...
        <div className="pos_container" style={{background:"#fff"}}>
            ...
            <div style={{marginTop:"15px"}}>
              ...
              <NavLink exact to="/dashboard"  ><WestIcon onClick={handleBack}/></NavLink>
                {/* </IconButton> */}
            </div>
                  <div>
                      <div>
                          <table align='center' cellPadding="5px" style={{ padding: "10px", width: '100%' }}>
                          <thead >
                              <tr>
                                <th >ITEM</th>
                                <th>PRICE</th>
                                <th  style={{width:"120px",textAlign:"center"}}>QNT.</th>
                                <th>TOTAL</th>
                              </tr>
                            </thead>
                            <tbody>
                            ...
                            order.orderItems.map(item=>{
                              return(
                                <tr style={{borderBottom:"1px solid rgb(164 164 164)"}}>
                                  <td >{item.name}</td>
                                  <td>{item.price}</td>
                                  <td align="start" style={{width:"100px",display:"flex",justifyContent:"space-between",alignItems:"center",marginRight:"15px"}}>
                                    <button className="add_btn" onClick={()=>handleRemove(item._id?item._id:item.id)}><RemoveIcon/></button>
                                    <span style={{margin:"0px 8px"}}>{item.quantity}</span>
                                    <button className="add_btn" onClick={()=>handleAdd(item._id?item._id:item.id)} ><AddIcon/></button>
                                  </td>
                                  <td>{(item.quantity * item.price).toFixed(2)}</td>
                                 {false&& <td>
                                    {/* <button className="btn" onClick={()=>deleteItem(item.name)}>X</button> */}
                                    <IconButton aria-label="delete" size="small" color="error" onClick={()=>deleteItem(item._id)}>
                                       <DeleteIcon />
                                   </IconButton>
                                  </td>}
                                </tr>
                            ...
                            :""
                              ...
                              <tr>
                                <td colSpan="3" align="end">Sub Total:</td>
                                <td><b>{order?(order.totalPrice-order.taxPrice).toFixed(2):" "}</b></td>
                              </tr>
                              <tr style={{borderBottom:"2px solid rgb(164 164 164)"}}>
                                  <td colSpan="3" align="end">Tax:</td>
                                <td>{order?(order.taxPrice).toFixed(2):" "}</td>
                              </tr>
                              <tr>
                                <td colSpan="3" align="end">Total:</td>
                                <td><b style={{fontSize:'1.4em'}}>{order?(order.totalPrice).toFixed(2):" "}</b></td>
                              </tr> 
         ...
         {!isPayment? <div className="products" style={{background:"transparent"}}>
                {/* <div className="cat_container"> */}
                {/* .filter(cat=>!cat.isAddOn) */}
                <div  className="cat_cont" >
                    {cat && cat.map((category) => (
                      <div onClick={() => categoryClickHandler(category.name, category._id?category._id:category.id,category.isAddOn)}  >
                        {(category.image&& false)?<div  className="img_container">
                       <img  style={{height:"100%",width:"100%"}} alt="ab" src={`${baseURL}/` + category.image}
                          onError={imageOnErrorHandler}
                          />
                         
                        </div>
                         :
                         <div className="chip" style={{backgroundColor:catId===(category._id?category._id:category.id)?"#f29a2e":"#32473a"}}>
                          {category.name}{category.isAddOn?<br/>:""}
                          {category.isAddOn?<><div className="adons-chip">Add</div></>:""}</div>
                     }
                        <div>
                          {(category.image&& false)?category.name:""}
                        </div>
                      </div>
                    ))}
                    </div>
                ...
                <div >
                  <div className="search">
                    <SearchIcon/>
                    <input type="text"  className="search_input" placeholder="Search Item" onChange={handleSearch}/>
                  </div>
                </div>
            ...
            : placeOrder?
            <div style={{width:"57%",height:"100vh",display:"inline-block",textAlign:"center",background:"#fff"}}>
              <h4>Pay Thorugh</h4>
                    <div>
                    <Button variant="contained" color="info" style={{height:"50px",margin:"20px"}} onClick={handleCash}>Cash</Button>
                    <Button variant="contained" color="info" style={{height:"50px",margin:"20px"}} onClick={()=>setPlaceOrder(false)}>Credit Card</Button>
                    <Button variant="contained" color="info" style={{height:"50px",margin:"20px"}} onClick={()=>setPlaceOrder(false)}>Debit Card</Button>
                    
                    </div>
                    <div>
                        <Button variant="contained" color="info" style={{height:"50px",margin:"20px"}} onClick={()=>setPlaceOrder(false)}>Check</Button>
                        <Button variant="contained" color="info" style={{height:"50px",margin:"20px"}} onClick={()=>setPlaceOrder(false)}>Gift Card</Button>
                        <Button variant="contained" color="info" style={{height:"50px",margin:"20px"}} onClick={()=>setPlaceOrder(false)}>Store Credit</Button>
                    </div>
            </div>
            :
            <div style={{width:"56%",background:"#fff"}}>
               <div  style={{width:"100%",display:"flex",justifyContent:"space-around",alignItems:"center"}}>
                    <Button variant="contained" color="error" onClick={cancleOrder}>Cancel</Button>
                    <Button variant="contained" color="success" onClick={createOrder}>Place Order</Button>
                </div>
            </div>
}
         </div>
