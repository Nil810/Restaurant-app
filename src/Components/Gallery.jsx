...
function Gallery({ onSelectImage, searchQuery }) {
  ...
  useEffect(() => {
    axios
      .get("https://api.menulive.in/api/products/images")
      .then((response) => {
  ...
  const totalPages3 = Math.ceil(filteredImages.length / 10);
  //Mojahid added
  const itemsPerPage2 = 10;
  const startIndex2 = (currentPage2 - 1) * itemsPerPage2;
  const endIndex2 = startIndex2 + itemsPerPage2;
  const filteredImagesPerPage = filteredImages.slice(startIndex2, endIndex2);
