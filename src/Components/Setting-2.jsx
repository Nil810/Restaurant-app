...
const Setting = () => {
  ...
  const handleChange = (event: React.SyntheticEvent, newValue) => {
    //this function updates the state of value based on the  new
    setValue(newValue); // each function is designed to handle changes in input fields (like text or colors) and update the corresponding state variables.
  ...
  let userData = sessionStorage.getItem("userData") //session storage stores the data for the duration of the page session
    ? JSON.parse(sessionStorage.getItem("userData")) //getItem("userData") checks if any data associated with "userData" key exist in the session storage
    : "";
  //sessionStorage stores data as strings, but often this data is in JSON format.
  //JSON.parse() converts this string back into an object that can be used in JavaScript.
  console.log(
    "Hi this is console from user data line 85 in settings",
    userData
  );
  const userId = userData ? userData.sub : " "; //This accesses the sub property of the userData object.

  //Defines a function called handleSubmit which will be executed when a user
  //submits a form or takes some action that requires submitting data.
  const handleSubmit = () => {
    console.log("hello, this is console from the handleSubmit");
    if (providerInfoId) {
      //Checks if providerInfoId exists. If it does, the code will execute the block that updates existing settings via a PUT request.
      if (merchantCode) {
        ...
        console.log("err"); //If merchantCode is not provided, it logs "err" to indicate that the submission failed due to missing data.
      }
    } else {
      if (merchantCode) {
        axios
          ...
          /**
           * Handles the response from the POST request if successful,
           * logging the result and fetching updated user information
           * similarly to the PUT request.
           * */
          .then((res) => {
            console.log(res);
  ...
  function handleProviderEdit(infoId) {
    ...
    setLatitude(fltData[0].lat || ""); // Set Latitude
    setLongitude(fltData[0].long || ""); // Set Longitude
      ...
      >
        ...
        <div style={{ padding: "20px" }}>
          ...
          <div className="row">
            ...
            {/* here the filter for the check for non veg in edit button */}
            <div className="col">
              <FormGroup>
                <FormControlLabel
                  control={<Checkbox />}
            ...
            {/* edit for aling order panel (pos) */}
            <div className="col">
              <FormGroup>
                <FormControlLabel
                  control={<Checkbox />}
          ...
          {/* edit for cancel and save button */}
          <div
            style={{
              width: "100%",
              textAlign: "right",
      ...
      <TabContext value={value}>
        ...
        >
          <TabList onChange={handleChange} aria-label="lab API tabs example">
            ...
            <Tab label="EXTERNAL SOURCES" value="2" />
